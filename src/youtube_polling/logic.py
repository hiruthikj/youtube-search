...
async def fetch_from_yt() -> dict:
    ...
    max_results = 10
    order = "date"
    published_after = datetime.now(tz=timezone.utc).replace(tzinfo=None).isoformat() + "Z"
    ...
    try:
        ...
        logging.debug(f"Response from Youtube API: status={response.status_code}, json={response.json()}")
    ...
    for item in result["items"]:
        ...
        await create_youtube_video_record(db=SessionLocal(), youtube_video=youtube_video)
